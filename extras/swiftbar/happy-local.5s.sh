...
# <xbar.title>Happy Stacks (legacy filename)</xbar.title>
...
# ============================================================================
# Menu: faster + clearer stack/component layout
# ============================================================================
...
MAIN_COLLECT="$(collect_stack_status "$MAIN_PORT" "$CLI_HOME_DIR" "com.happy.local" "$HAPPY_HOME_DIR")"
...
render_stack_info "" "main" "$MAIN_PORT" "$MAIN_SERVER_COMPONENT" "$HAPPY_HOME_DIR" "$CLI_HOME_DIR" "com.happy.local" "" "$TAILSCALE_URL"
render_component_server "" "main" "$MAIN_PORT" "$MAIN_SERVER_COMPONENT" "$MAIN_SERVER_STATUS" "$MAIN_SERVER_PID" "$MAIN_SERVER_METRICS" "$TAILSCALE_URL" "com.happy.local"
...
render_component_autostart "" "main" "com.happy.local" "$MAIN_LAUNCHAGENT_STATUS" "$MAIN_AUTOSTART_PID" "$MAIN_AUTOSTART_METRICS" "$LOGS_DIR"
...
if [[ -z "$PNPM_BIN" ]]; then
  ...
  echo "--Sync mirrors (all) | bash=$PNPM_BIN param1=wt param2=sync-all dir=$HAPPY_LOCAL_DIR terminal=true refresh=true"
  echo "--Update all (dry-run) | bash=$PNPM_BIN param1=wt param2=update-all param3=--dry-run dir=$HAPPY_LOCAL_DIR terminal=true refresh=true"
  echo "--Update all (apply) | bash=$PNPM_BIN param1=wt param2=update-all dir=$HAPPY_LOCAL_DIR terminal=true refresh=true"
fi
...
if [[ -z "$PNPM_BIN" ]]; then
  ...
  echo "--Mobile dev helper | bash=$PNPM_BIN param1=mobile dir=$HAPPY_LOCAL_DIR terminal=true"
fi
